...
class Spp extends CI_Controller
{
    ...
    {
        $data['judulHalaman']   = 'Pembayaran Uang SPP';
        $data['halaman']        = 'invoice';
        $data['level']          = 'wali';
        $data['setting']        = $this->db->get('setting')->result();
        $data['invoices']       = $this->db->get_where('spp', array('student_id' => $this->db->get_where('siswa', array('parent_id' => $this->session->userdata('parent_id')))->row()->student_id))->result_array();

        $this->load->view('backend/temp/header', $data);
        $this->load->view('backend/wali/sidebar', $data);
        $this->load->view('backend/wali/spp', $data);
    ...
    public function buat_tagihan()
    {
        ...
        redirect(base_url('wali/spp'), 'refresh');
    }
